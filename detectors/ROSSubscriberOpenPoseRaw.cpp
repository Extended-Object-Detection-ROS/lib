...
namespace eod{
    ...
    ROSSubscriberOpenPoseRaw::ROSSubscriberOpenPoseRaw(float kpt_score_th, int num_paf_samples, float min_paf_score_th, float paf_sample_th) {
    ...
    std::vector<ExtendedObjectInfo> ROSSubscriberOpenPoseRaw::Detect2(const InfoImage& image, int seq){
        ...
        if (parent_base->inner_storage.find("nn_raw_output") == parent_base->inner_storage.end()){
            printf("No data in nn_raw_output of inner storage!\n");
            return results;
        }
        
        depthai_ros_extended_msgs::NeuralNetworkRawOutputConstPtr msg = *(depthai_ros_extended_msgs::NeuralNetworkRawOutputConstPtr*)parent_base->inner_storage["nn_raw_output"];
